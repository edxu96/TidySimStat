...
function [b, sState] = simDiscreteEvent(clockSimZero, nServer, nEvent, funcArrive, funcServe, vecParaArrive, ...
    ...
    sState(1).intervalArrive = funcArrive(vecParaArrive);
    sState(1).timeServe = funcServe(vecParaServe);
    sState(1).clockSim = clockSimZero + sState(1).intervalArrive;
    ...
    for i = 2:nEvent
        ...
        sState(i).intervalArrive = funcArrive(vecParaArrive);
        sState(i).timeServe = funcServe(vecParaServe);
        sState(i).clockSim = sState(i-1).clockSim + sState(i).intervalArrive;
