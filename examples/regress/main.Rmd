...
```{r, echo=T}
mods[[1]] %>% test_white(dat, resi2 ~ x2 + I(x2^2), 2) %>% tab_ti(T)
```
...
```{r}
mods[[1]] %>% test_reset(dat) %>% tab_ti(T)
```
...
test_lik(mods[[4]], mods_lik[[1]]) %>% tab_ti(T)
```
...
test_lik(mods[[4]], mods_lik[[2]]) %>% tab_ti(T)
```

```{r}
test_lik(mods_lik[[1]], mods_lik[[2]]) %>% tab_ti(T)
```
...
```{r}
AIC(mods[[7]], mods[[6]], mods[[3]]) %>% tab_ti(T)
```
...
![Illustration of the model structure.](./1.png)
