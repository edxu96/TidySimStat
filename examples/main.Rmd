...
## 8. Mis-Specification Analysis of `mods[[5]]` and `mods[[4]]`

```{r, echo = T}
mods[[5]]$residuals %>% 
  tseries::jarque.bera.test()%>%  
  {(.$p.value <= qchisq(0.95, summary(mods[[5]])$df[1], lower.tail = TRUE, 
    log.p = FALSE))}
```
...
dat_2 %>%
  ...
  {lm(resi2 ~ x2 + x3 + x4 + x6 + I(x2^2) + I(x3^2) + I(x4^2) + I(x6^2), 
    data = .)} %>%
  {summary(.)$r.squared} %>%
  {. * nrow(dat_2) <= qchisq(0.95, 5, lower.tail = TRUE, log.p = FALSE)}
```
...
dat_2 %>%
  ...
  {lm(resi2 ~ x2 + x3 + x4 + x6 + I(x2^2) + I(x3^2) + I(x4^2) + I(x6^2) +
    ...
    I(x4 * x6), data = .)} %>%
  {summary(.)$r.squared} %>%
  {. * nrow(dat_2) <= qchisq(0.95, 15, lower.tail = TRUE, log.p = FALSE)}
```

```{r}
dat_2 %>%
  mutate(y_hat = fitted(mods[[5]])) %>%
  {lm(y ~ x2 + x3 + x4 + x6 + I(y_hat^2), data = .)} %>%
  lmSupport::modelEffectSizes() %>%
  {.$Effects} %>%
  {.[nrow(.), 3] * (nrow(dat_2) - 1)} %>%
  {1 - pchisq(., 1)} %>%
  {. >= 0.05}  # If true, the num hypothesis is rejected.
```

`mods[[4]]` has passed all tests.

```{r}
mods[[4]]$residuals %>% 
  tseries::jarque.bera.test()%>%  
  {(.$p.value <= qchisq(0.95, summary(mods[[4]])$df[1], lower.tail = TRUE, 
    log.p = FALSE))}
```

```{r, echo = T}
dat_2 %>%
  mutate(resi2 = mods[[4]]$residuals^2) %>%
  {lm(resi2 ~ x2 + x3 + x4 + x5 + x6 + x7 + I(x2^2) + I(x3^2) + I(x4^2) + 
    I(x5^2) + I(x6^2) + I(x5^2), data = .)} %>%
  {summary(.)$r.squared} %>%
  {. * nrow(dat_2) <= qchisq(0.95, 7, lower.tail = TRUE, log.p = FALSE)}
```
...
dat_2 %>%
  mutate(resi2 = mods[[4]]$residuals^2) %>%
  {lm(resi2 ~ x2 + x3 + x4 + x5 + x6 + x7 + I(x2^2) + I(x3^2) + I(x4^2) + 
    I(x5^2) + I(x6^2) + I(x7^2) + I(x2 * x3) + I(x2 * x4) + I(x2 * x5) + 
    I(x2 * x6) + I(x2 * x7) + I(x4 * x3) + I(x5 * x3) + I(x6 * x3) + 
    I(x3 * x7) + I(x4 * x5) + I(x4 * x6) + I(x4 * x7) + I(x5 * x6) + 
    I(x5 * x7) + I(x6 * x7), data = .)} %>%
  {summary(.)$r.squared} %>%
  {. * nrow(dat_2) <= qchisq(0.95, 28, lower.tail = TRUE, log.p = FALSE)}
```
...
mods[[4]] %>%
  resettest(power = 2:3, type = c("fitted", "regressor", "princomp"), 
    data = dat_2) %>%
  {.$p.value <= qchisq(0.95, 6, lower.tail = TRUE, log.p = FALSE)}
```

## 9. Mis-Specification Analysis of `mods[[3]]`
...
mods[[3]]$residuals %>% 
  tseries::jarque.bera.test()%>%  
  {(.$p.value <= qchisq(0.95, summary(mods[[3]])$df[1], lower.tail = TRUE, 
    log.p = FALSE))}
```
