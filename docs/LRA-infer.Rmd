...
From section 1-3-4, 2-3-2 in [hendry2007econometric](#reference):
...
```R
qchisq(0.95, df, lower.tail = TRUE, log.p = FALSE)
```

#### LLR-Test for One Parameter {-#LLR-Test-1}
...
#### LLR-Test for More Parameters {-#LLR-Test-N}

```{r}
mods_lik <- list()
mods_lik[[1]] <- lm(y ~ x2 + x3 + x4 + x6 + x7, data = dat_2)
test_lik(mods[[4]], mods_lik[[1]]) %>% tab_ti()
```
...
test_lik(mods[[4]], mods_lik[[2]]) %>% tab_ti()
```
